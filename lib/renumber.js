...
all.forEach((s, i) => {
  const want = String(i + 1 - OFFSET);
  const pn = s.children.find(c => c.type === 'TEXT' && Math.abs(c.x - 1712) < 5 && Math.abs(c.y - 990) < 5);
...
return { dryRun: DRY_RUN, total: all.length, offset: OFFSET,
